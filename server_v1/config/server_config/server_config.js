...
class ServerApp {
    constructor (params) {
        ...
        this.localIP = LocalIPAddress; // 本机 IP
        this.rootPath = params.rootPath || '/'; // 根目录路径
        this.port = params.port; // 要开启服务的端口号
        this.port_https = Number(params.port) - 2000; // 要开启服务的端口号 https 端口
    ...
    log4jsMiddleware () {
        ...
        _this.app.use(
            log4js.connectLogger(_this.logger, {
                level: 'auto',
                format: `[:remote-addr] [:method] [HTTP/:http-version] [:status] [:response-timems] [:hostname] [:url] [:referrer] [:user-agent] [:content-length]`
            })
        );
    }
    ...
    useCustomMiddleware () {
        ...
        for (const key in _this.customMiddleware) {
            _this.app.use(key, (request, response, next) => {
    ...
    uploadRequest () {
        ...
        let upload = multer({
            ...
            fileFilter (req, file, callback) {
                ...
                /image/.test(file.mimetype) ? callback(null, true) : callback(null, false);
            }
        });
        ...
        // 上传文件处理
        _this.app.post('/upload', cpUpload, (request, response, next) => {
                        ...
                        response.sendFile(path.join(_this.rootPath, request.path), {}, (error) => {
                            ...
                            if (error) {
                                if (error.statusCode === 404 && _this.page404.enabled && _this.page404.page404Type.test(request.path)) {
                                    ...
                                    // 响应 404 状态码
                                    response.status(404).send(`Not Found</br>${error}</br><script>let error404 = ${JSON.stringify(error)}</script>`);
                                }
                            }
                        });
                    }
    ...
    openServer () {
        ...
        const filePath_Relative = path.relative(__dirname, '/node_server/https_server');
        const filePath_Absolute = path.normalize(__dirname + filePath_Relative);
        ...
        // HTTPS
        _this.server_https = https.createServer(options, _this.app).listen(_this.port_https, () => {
