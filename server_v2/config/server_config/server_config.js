...
const { createProxyMiddleware, Filter, Options, RequestHandler, responseInterceptor } = require('http-proxy-middleware'); // 代理服务

// 导入模块文件
const { siteConfigArray } = require('../server_list_options/server_list_options.js'); // 站点配置文件
...
class ExpressRouter {
    ...
    proxyRequest () {
        ...
        _this.proxy.forEach((element, index) => {
            if (element.enabled) {
                ...
                _this.router.use('/', createProxyMiddleware(element.proxyPath, createProxyOptions));
            }
        });
    }
    ...
    lastAllResponse () {
        ...
        _this.router.all('*', (request, response, next) => {
